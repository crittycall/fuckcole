...
export default new (class serverRotations {
  constructor() {
    ...
    register("packetSent", (packet, event) => {
      ...
      if (!this.fakePacketSent) {
        if (simpleName == "C03PacketPlayer" || simpleName == "C05PacketPlayerLook") Client.sendPacket(new C05PacketPlayerLook(this.yaw, this.pitch, packetData.onGround));
        else if (simpleName == "C04PacketPlayerPosition" || simpleName == "C06PacketPlayerPosLook") Client.sendPacket(new C06PacketPlayerPosLook(packetData.x, packetData.y, packetData.z, this.yaw, this.pitch, packetData.onGround));

        this.fakePacketSent = true;
      } else if (this.fakePacketSent) {
        if (simpleName == "C03PacketPlayer" || simpleName == "C05PacketPlayerLook") Client.sendPacket(new C03PacketPlayer(packetData.onGround));
        else if (simpleName == "C04PacketPlayerPosition" || simpleName == "C06PacketPlayerPosLook") Client.sendPacket(new C04PacketPlayerPosition(packetData.x, packetData.y, packetData.z, packetData.onGround));
      }
  ...
  setRotation(yaw, pitch, callback = null) {
    ...
    if (Player.getRawYaw() + (yaw - Player.getYaw()) == this.yaw && pitch == this.pitch && this.shouldRotate) return;
