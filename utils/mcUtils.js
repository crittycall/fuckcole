import { traverseVoxels, Vec3, MovingObjectPosition } from "../../BloomCore/utils/Utils";
import { MathHelper, mc, Float, PlayerControllerMP } from "./utils";

const rightClickDelayTimer = mc.getClass().getDeclaredField("field_71467_ac");
...
export class McUtils {
  static setRightClickDelayTimer(tick) {
    rightClickDelayTimer.setInt(mc, tick);
  }

  static getRightClickDelayTimer() {
    return rightClickDelayTimer.get(mc);
  }
  ...
  static getAngles(x, y, z, calcYaw, calcPitch, currentYaw = Player.getYaw(), currentPitch = Player.getPitch(), forceSneak = false) {
    ...
    if (calcPitch) pitchToMove = -Math.atan2(y - Player.getRenderY() - Player.getPlayer().func_70047_e() - (!Player.isSneaking() && forceSneak ? sneakHeight : 0), Math.sqrt(tvx * tvx + tvz * tvz)) * radToDeg - currentPitch;
  ...
  static rayTraceBlock(maxDistance = 50, partialTicks = 1, forceSneak = false, yaw = Player.getYaw(), pitch = Player.getPitch()) {
    ...
    for (let i = 0; i < maxDistance; i++) {
      ...
      let hit = [eyeX + dx * c, eyeY + dy * c, eyeZ + dz * c].map((coord) => Math.round(coord * 1e10) * 1e-10);
      ...
      if (McUtils.getBlock(x, y, z)?.type?.getID() !== 0) return hit;
    }
  ...
  static getClosesetCoords3D(pos, min, max) {
    ...
    return new Vec3(Math.max(min.field_72450_a, Math.min(pos.field_72450_a, max.field_72450_a)), Math.max(min.field_72448_b, Math.min(pos.field_72448_b, max.field_72448_b)), Math.max(min.field_72449_c, Math.min(pos.field_72449_c, max.field_72449_c)));
  }
  ...
  static getClosesetMOPOnBlock(eyePos, blockPos, world) {
    ...
    let closest = McUtils.getClosesetCoords3D(eyePos, new Vec3(aabb.field_72340_a, aabb.field_72338_b, aabb.field_72339_c), new Vec3(aabb.field_72336_d, aabb.field_72337_e, aabb.field_72334_f));
    ...
    if (eyePos.field_72450_a === closest.field_72450_a && eyePos.field_72448_b === closest.field_72448_b && eyePos.field_72449_c === closest.field_72449_c) {
      ...
      const diagonal = Math.sqrt( (aabb.field_72336_d - aabb.field_72340_a) ** 2 + (aabb.field_72337_e - aabb.field_72338_b) ** 2 + (aabb.field_72334_f - aabb.field_72339_c) ** 2) + 1;
  ...
  static useEntity(movingObject) {
    ...
    if (mc.field_71442_b.func_178894_a(thePlayer, entityHit, movingObject)) flag = false;
    else if (mc.field_71442_b.func_78768_b(thePlayer, entityHit)) flag = false;

    if (flag) {
      const itemstack = thePlayer.field_71071_by.func_70448_g();

      if (itemstack !== null && mc.field_71442_b.func_78769_a(thePlayer, theWorld, itemstack)) mc.field_71460_t.field_78516_c.func_78445_c();
    }
  }
}